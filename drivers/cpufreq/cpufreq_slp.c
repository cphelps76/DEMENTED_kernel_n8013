...
unsigned int prev_freq_step1;
unsigned int prev_sampling_rate1;
static void cpufreq_pegasusq_early_suspend(struct early_suspend *h)
{
	...
	dbs_tuners_ins.early_suspend =
		atomic_read(&g_hotplug_lock);
#endif
	prev_freq_step1 = dbs_tuners_ins.freq_step;
	prev_sampling_rate1 = dbs_tuners_ins.sampling_rate;
...
static void cpufreq_pegasusq_late_resume(struct early_suspend *h)
{
	...
	dbs_tuners_ins.freq_step = prev_freq_step1;
	dbs_tuners_ins.sampling_rate = prev_sampling_rate1;
#if EARLYSUSPEND_HOTPLUGLOCK
	apply_hotplug_lock();
	start_rq_work();
#endif
